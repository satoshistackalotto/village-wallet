...
export default function VillageWallet() {
      ...
      if (selectedNetwork) {
        try {
          ...
          for (const rpc of rpcsToTry) {
            try {
              console.log(`ðŸ” Trying ${network.name} ${rpc.label}: ${rpc.url}`);
              ethProvider = new ethers.providers.JsonRpcProvider(rpc.url);
              await ethProvider.getBlockNumber();
              console.log(`âœ… Connected to ${network.name} (${rpc.label})`);
              connected = true;
              break; // Success! Stop trying
            } catch (rpcError) {
              console.log(`âŒ ${rpc.label} failed, trying next...`);
              continue; // Try next RPC
            }
          }
          
          if (!connected) {
            console.log(`âŒ All RPCs failed for ${network.name}`);
            Alert.alert(
              'Connection Failed',
          ...
          console.error(`âŒ Failed to switch to ${selectedNetwork}:`, error);
  ...
  const initializeApp = async () => {
    try {
      ...
      for (const rpc of rpcsToTry) {
        try {
          console.log(`ðŸ” Initializing with ${network.name} ${rpc.label}`);
          ethProvider = new ethers.providers.JsonRpcProvider(rpc.url);
          await ethProvider.getBlockNumber();
          console.log(`âœ… Network initialized (${rpc.label})`);
          connected = true;
          break;
        } catch (rpcError) {
          console.log(`âŒ ${rpc.label} failed, trying next...`);
      ...
      if (!connected) {
        console.log('âŒ All RPCs failed during initialization');
      }
      ...
      try {
        ...
        if (supported) {
          ...
          setNfcEnabled(true);
          console.log('âœ… NFC initialized successfully');
        } else {
          console.log('âš ï¸ NFC not supported on this device');
        }
      } catch (nfcError) {
        // NFC failed, but that's okay - app can still work
        console.log('âš ï¸ NFC initialization failed (app will still work):', nfcError.message);
        setNfcEnabled(false);
      }
    } catch (error) {
  ...
  const resolveNameOrAddress = async (input: string) => {
    try {
      ...
      if (input.includes('.eth')) {
        ...
        console.log(`âœ… Resolved ${input} â†’ ${resolvedAddress}`);
        return { 
  ...
  const renderHomeScreen = () => (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>ðŸ˜ï¸ Village Wallet</Text>
      ...
      <View style={styles.buttonContainer}>
        ...
        >
          <Text style={styles.buttonText}>ðŸ’° Receive Payment</Text>
        ...
        >
          <Text style={styles.buttonText}>ðŸ“Š Check Balance</Text>
      ...
      {!nfcEnabled && (
        <View style={styles.warning}>
          <Text style={styles.warningText}>âš ï¸ NFC is disabled. Enable in settings.</Text>
        </View>
      )}
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        ...
        {amount && parseFloat(amount) > 0 && (
          <Text style={styles.usdEstimate}>
            â‰ˆ ${(parseFloat(amount) * (selectedToken === 'usdt' || selectedToken === 'usdc' || selectedToken === 'dai' ? 1 : 0)).toFixed(2)} USD (est.)
          </Text>
        )}
      </View>
      ...
      >
        {loading ? (
          ...
          <Text style={styles.buttonText}>Ready for Payment â†’</Text>
        )}
      </TouchableOpacity>
    ...
    <View style={styles.container}>
      ...
      >
        {loading ? (
          ...
          <Text style={styles.buttonText}>Confirm Payment âœ“</Text>
        )}
      </TouchableOpacity>
      ...
      <View style={styles.warning}>
        <Text style={styles.warningText}>
          ðŸ”’ Your PIN is secure. The merchant cannot see it.
        </Text>
      </View>
    </View>
  );
        ...
        {cardData && (
          <View style={styles.balanceContainer}>
            ...
            <View style={[styles.infoBox, {marginTop: 20}]}>
              <Text style={styles.infoLabel}>ðŸ’¡ Tip:</Text>
              <Text style={styles.infoValue}>
                {cardData.ens 
      ...
      <View style={styles.container}>
        ...
        <View style={styles.section}>
          ...
          {merchantAddress ? (
            ...
            <View style={styles.warning}>
              <Text style={styles.warningText}>
                âš ï¸ No address set. Set your address to receive payments.
              </Text>
            </View>
          )}
          ...
          >
            {loading ? (
              ...
              <Text style={styles.buttonText}>ðŸ’¾ Save Address</Text>
            )}
          </TouchableOpacity>
          ...
          >
            <Text style={styles.buttonText}>ðŸ“± Scan from Card</Text>
          </TouchableOpacity>
        </View>
